...
function killBot(bot, index, headshot = false) {
  ...
  cameraShake += headshot ? 0.09 : 0.055;
...
function updateParticles() {
  for (let i = particles.length - 1; i >= 0; i--) {
    ...
    if (p.grow) {
      ...
      if (p.material.opacity !== undefined) p.material.opacity *= 0.88;
    } else {
...
function showHitmarker(headshot) {
  ...
  hitmarker.style.transform = "translate(-50%, -50%) scale(1.45)";
